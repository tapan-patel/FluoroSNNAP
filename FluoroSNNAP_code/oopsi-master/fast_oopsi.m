...
if V.fast_iter_max>1;
    ...
    if ~isfield(V,'fast_plot'), V.fast_plot = 0; end    % whether to plot results from each iteration
...
%% MakePlot
    function MakePlot(n,F,P,V)
        if V.fast_plot == 1
            ...
            figure(FigNum+1),  ncols=V.Ncells; nrows=3; END=V.T; h=gobjects(V.Ncells,2);
