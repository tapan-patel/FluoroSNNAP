function [ica_segments, segmentlabel, segcentroid,L,Lbig,ica_filtersbw] = CellsortSegmentation(ica_filters, smwidth, thresh, arealims, plotting)
...
for j = 1:nic
    ...
    nsegs = nsegs + length(Lbig);
end
ica_segments = false(nsegs,r,c);

for j = 1:nic
    abort=multiWaitbar('Performing segmentation',j/nic*.8,'Color','r');
    if(abort)
        return;
    end
    % Label contiguous components
...
%         ica_segments(jj+k,:,:) = ica_filtersuse .* ( 0*(L==0) + (L==Lbig(jj)) );  % Exclude background
        ica_segments(jj+k,:,:) = L==Lbig(jj);  % Exclude background
    end
